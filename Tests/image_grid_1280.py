...
def process_subdirectory(subdirectory):
    ...
    for root, dirs, files in os.walk(subdirectory):
        for file in files:
            if file.endswith(".png") and '1216' not in file:  # ignore files with '512' in their name
...
def main(directory):
    ...
    for root, subdirs, files in os.walk(directory):
        for subdir in subdirs:
            ...
            if images:
                ncols = 6  # change the number of columns back to 6
                grid = make_grid(images, ncols)
                grid_name = f"{os.path.basename(subdir_path)}_image_grid.jpg"  # change file name here
...
if __name__ == "__main__":
    directory = '/home/gkirilov/Checkpoint/SD_2.1/'  # Put your directory path here
    main(directory)